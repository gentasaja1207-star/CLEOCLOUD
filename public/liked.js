    ...
    render() {
        ...
        if(liked.length > 0) {
            likedHtml = liked.map(function(s, i) {
                ...
                var cardBg = isPlay ? 'bg-[rgba(255,255,255,0.14)] border border-white/40 shadow-xl' : (isCur ? 'bg-[rgba(255,255,255,0.12)] border border-white/30' : 'backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 hover:bg-[rgba(255,255,255,0.1)]');
                ...
                return '<div class="rounded-2xl '+cardBg+' p-2.5 flex items-center gap-3 active:scale-[0.98] transition-all duration-300 group shadow-lg shadow-black/25">'+
                    '<div onclick="PK(\'liked\','+i+')" class="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">'+
                        '<img loading="lazy" decoding="async" src="'+s.cover+'" class="w-14 h-14 rounded-xl object-cover shrink-0 shadow-md border border-white/10" onerror="this.src=\''+FI+'\'" />'+
        ...
        el.innerHTML = `
        <div class="pt-8 pb-3.5 px-4 sticky top-0 z-30 border-b border-white/[0.08] transition-all flex justify-between items-center cc-header-blur">
