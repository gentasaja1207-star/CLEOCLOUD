// ============================================================
// CLEO CLOUD - CORE PLAYER (FULL FIX)
// ============================================================
...
const S={ht:[],sr:[],ar:[],hc:[],hcp:[],hca:[],sq:'',filter:'all',ct:null,pl:[],pi:-1,ps:'',ip:false,il:false,rm:'all',isShuffle:false,currentAccentColor:'#ec4899',autoNext:true,iv:null,pt:0,pd:0,at:'home',ld:{type:'none',lines:[]},cli:-1,lo:false,lyricOffset:0,playbackRate:1.0,sleepSecondsLeft:0,sleepEndWithTrack:false,volume:1.0,lastVolume:1.0,
    // --- FIX: flag utk bedain pause krn user tap tombol vs pause krn OS/interupsi (lock screen, panggilan telepon, dsb) ---
    userPaused:false};
...
function updateOG(title,image){
    var t=document.querySelector('meta[property="og:title"]');if(!t){t=document.createElement('meta');t.setAttribute('property','og:title');document.head.appendChild(t);}t.setAttribute('content',title+' | Cleo Cloud');
    ...
    document.title=title+' - Cleo Cloud';
}
...
AU.addEventListener('play',function(){S.ip=true;S.il=false;S.userPaused=false;resumeRetryCount=0;UB();SP();try{AU.playbackRate=S.playbackRate||1.0;}catch(ex){}});
AU.addEventListener('pause',function(){
    if(AU.ended)return;
    // FIX (lagu berhenti sendiri / notifikasi hilang): kalau user TIDAK sengaja menekan pause,
    // event 'pause' ini kemungkinan besar berasal dari interupsi OS (lockscreen, telepon masuk,
    // audio app lain, dsb). Jangan langsung nyerah -> coba resume otomatis dulu beberapa kali,
    // baru kalau memang gagal, update UI/notifikasi jadi paused.
    if(!S.userPaused && S.ip){
        attemptAutoResume();
        return;
    }
    S.ip=false;UB();ST();
...
AU.addEventListener('playing',function(){S.il=false;resumeRetryCount=0;UB();});
...
AU.addEventListener('stalled',function(){ if(S.ip && !S.userPaused) attemptAutoResume(); });
AU.addEventListener('error',function(){
    if(!AU.src)return;
    // FIX: sebelumnya error langsung nyerah & bikin notifikasi/player mati. Sekarang coba
    // reload ulang track (link proxy/audio YouTube kadang expired) sebelum benar2 menyerah.
    if(S.ct && S.ip && !S.userPaused && resumeRetryCount < MAX_RESUME_RETRIES){
        resumeRetryCount++;
        var resumeAt = S.pt || 0;
        setTimeout(function(){ if(S.ct) loadTrack(S.ct, resumeAt); }, 800 * resumeRetryCount);
        return;
    }
    S.il=false;S.ip=false;UB();
});

// ---- FIX: RESILIENSI BACKGROUND PLAYBACK / NOTIFICATION ----
var resumeRetryCount = 0;
var MAX_RESUME_RETRIES = 4;
function attemptAutoResume(){
    if(resumeRetryCount >= MAX_RESUME_RETRIES){
        // Sudah dicoba berkali-kali dan tetap gagal -> baru benar2 update state jadi paused,
        // biar notifikasi & UI tidak "menggantung" playing padahal audio diam.
        S.ip=false;UB();ST();
        resumeRetryCount=0;
        return;
    }
    resumeRetryCount++;
    var delay = Math.min(300 * resumeRetryCount, 1500);
    setTimeout(function(){
        if(S.userPaused || !S.ct) return; // user sempat pause manual di antara retry, jangan paksa play
        var p = AU.play();
        if(p && p.catch){ p.catch(function(){ /* akan dicoba lagi lewat event pause berikutnya */ }); }
    }, delay);
}

// Saat app kembali ke foreground (buka lagi dari background/lockscreen), sinkronkan ulang
// notifikasi & lock screen supaya tidak "hilang" - dan kalau ternyata audio sempat berhenti
// padahal harusnya masih playing, coba lanjutkan otomatis.
document.addEventListener('visibilitychange', function(){
    if(document.visibilityState === 'visible'){
        if(S.ct){
            updateMediaSessionMetadata(S.ct);
            updateMediaSessionPlaybackState();
        }
        if(S.ip && !S.userPaused && AU.paused && S.ct){
            attemptAutoResume();
        }
        savePlaybackSession();
    }
...
// Simpan posisi & queue secara berkala + saat halaman mau ditutup/dibekukan, supaya kalau
// browser/iOS benar2 mematikan halaman di background, sesi (lagu, posisi, queue) tidak hilang
// total dan bisa langsung dilanjutkan begitu app dibuka lagi (bukan reset ke awal).
function savePlaybackSession(){
    try{
        if(!S.ct)return;
        localStorage.setItem('cleo_playback_session', JSON.stringify({
            ct:S.ct, pl:S.pl, pi:S.pi, ps:S.ps, pt:S.pt, ip:S.ip, rm:S.rm, isShuffle:S.isShuffle, savedAt:Date.now()
        }));
    }catch(e){}
}
setInterval(savePlaybackSession, 5000);
document.addEventListener('pagehide', savePlaybackSession);
window.addEventListener('beforeunload', savePlaybackSession);

// Saat app dibuka kembali (termasuk setelah di-suspend total oleh OS), pulihkan info lagu
// terakhir supaya mini player langsung tampil lagi tanpa user harus mencari ulang.
// Tidak auto-play (browser tidak izinkan autoplay tanpa gesture user), tapi tinggal 1x tap play.
function restorePlaybackSession(){
    try{
        var raw = localStorage.getItem('cleo_playback_session');
        if(!raw)return;
        var d = JSON.parse(raw);
        if(!d || !d.ct || (Date.now() - (d.savedAt||0)) > 12*60*60*1000) return; // basi kalau >12 jam
        S.ct=d.ct;S.pl=d.pl||[d.ct];S.pi=typeof d.pi==='number'?d.pi:0;S.ps=d.ps||'direct';
        S.pt=d.pt||0;S.rm=d.rm||'all';S.isShuffle=!!d.isShuffle;S.ip=false;S.userPaused=true;
        if(typeof UU==='function')UU();
        if(typeof MP!=='undefined'&&MP.show)MP.show();
        updateMediaSessionMetadata(S.ct);
    }catch(e){}
}
if(document.readyState==='complete'||document.readyState==='interactive'){restorePlaybackSession();}
else{document.addEventListener('DOMContentLoaded', restorePlaybackSession);}
...
if('mediaSession' in navigator){
    try{
        ...
        navigator.mediaSession.setActionHandler('stop',function(){S.userPaused=true;try{AU.pause();}catch(e){}});
...
// FIX (queue/shuffle konsisten): satu-satunya tempat yang menentukan "lagu berikutnya".
// Hasilnya di-cache per S.pi supaya preview (mini transition, preload) dan lagu yang BENERAN
// diputar pas track abis selalu sama persis - nggak ada lagi 2 perhitungan next yang beda sendiri2.
var cachedNextIndex = -1;
var cachedNextIndexForPi = -1;
function peekNextIndex(){
    if(!S.pl || !S.pl.length) return -1;
    if(cachedNextIndexForPi === S.pi && cachedNextIndex > -1 && cachedNextIndex < S.pl.length){
    ...
    if(S.isShuffle && S.pl.length > 1){
        ni = S.pi;
        var attempts = 0;
        while(ni === S.pi && attempts < 10){
            ni = Math.floor(Math.random() * S.pl.length);
            attempts++;
        }
    } else {
        ni = S.pi + 1;
...
async function triggerPreloadNextTrack(){
    ...
    try {
        ...
        var nextTrack = null;
        // FIX (queue/shuffle nggak konsisten): sebelumnya di sini "next" buat preload SELALU
        // dihitung pl[pi+1], padahal kalau shuffle aktif, NX() yang beneran mutar nanti milih
        // index ACAK. Akibatnya lagu yg di-preload beda sama lagu yg kepilih pas lagu abis ->
        // kesannya "loncat random". Sekarang pakai helper yang sama persis dgn NX().
        if (!S.isShuffle && S.pl && S.pi + 1 >= S.pl.length && S.autoNext) {
            var fetched = await fetchAutoNextRecommendations(S.ct);
            if (!fetched) return;
        }
        var ni2 = peekNextIndex();
        if (ni2 === -1) return;
        nextTrack = S.pl[ni2];
...
function checkAutoNextTransition() {
    ...
    if (remaining > 0 && remaining <= windowSec) {
        var pIdx = peekNextIndex();
        var nextTrack = (pIdx > -1 && S.pl) ? S.pl[pIdx] : null;
...
function UB(){
    ...
    var accent = S.currentAccentColor || '#ec4899';
...
function updateOG(title, cover, artist) {
    if (title && cover) {
        ...
        var docTitle = fullTitle + ' | Cleo Cloud';
        var description = 'Dengarkan ' + fullTitle + ' di Cleo Cloud';
        ...
        document.title = 'Cleo Cloud';

        setMetaTag('og:title', 'Cleo Cloud', true);
        setMetaTag('og:description', 'Cleo Cloud - Web Music Player', true);
        setMetaTag('og:image', defaultCover, true);
        setMetaTag('og:image:width', '600', true);
        setMetaTag('og:image:height', '600', true);
        setMetaTag('og:url', location.href, true);
        setMetaTag('twitter:card', 'summary_large_image', false);
        setMetaTag('twitter:title', 'Cleo Cloud', false);
        setMetaTag('twitter:description', 'Cleo Cloud - Web Music Player', false);
...
function updateOGForArtist(artistName, coverUrl) {
    ...
    var docTitle = name + ' - Artist | Cleo Cloud';
    var description = 'Dengarkan lagu dan album terbaik dari ' + name + ' di Cleo Cloud';
...
function updateOGForAlbum(albumTitle, coverUrl, artistName) {
    ...
    var docTitle = fullTitle + ' - Album | Cleo Cloud';
    var description = 'Dengarkan album ' + fullTitle + ' di Cleo Cloud';
    var cover = (coverUrl && coverUrl !== FI) ? coverUrl : 'https://www.gobox.my.id/file/R0ym4wqfznmp.png';
...
async function fetchAudioAndPlay(track,resumeAt){
    ...
    try{
        ...
        if(audioUrl){
            var isSavedOffline = (typeof isOfflineSong === 'function') && isOfflineSong(track);
            var trulyOffline = !navigator.onLine;
            // Proxy (yang kena cache PWA) cuma dipake kalau BENERAN lagi offline atau equalizer aktif.
            // Kalau online, tetep pake link langsung kayak sebelumnya biar nggak numpangin ke serverless
            // function proxy yang bisa bikin masalah/lag sendiri pas nggak perlu.
            if ((isSavedOffline && trulyOffline) || (typeof audioCtx !== 'undefined' && audioCtx)) {
...
function TP(){
    if(!S.ct)return;
    if(!AU.src){
        // FIX: kalau ini hasil restore sesi (habis app dibuka lagi), lanjutkan dari posisi terakhir
        loadTrack(S.ct, S.pt>2?S.pt:0);
        return;
    }
    if(AU.paused){
        S.userPaused=false;resumeRetryCount=0;
        S.il=true;UB();
        var p = AU.play();
        if(p !== undefined && p.then){
            p.then(function(){
        ...
        S.userPaused=true; // pause sengaja ditekan user -> jangan auto-resume
        AU.pause();
        S.ip = false;
        S.il = false;
        UB();
...
async function fetchAutoNextRecommendations(track) {
    ...
    try {
        ...
        if (d && d.status && d.result && d.result.songs && d.result.songs.length > 0) {
            ...
            var existingIds = {};
            (S.pl||[]).forEach(function(s){ existingIds[s.videoId||s.id] = true; });
            var newSongs = d.result.songs.filter(function(s) {
                // FIX: jangan masukkan lagu yg sama persis / sudah ada di queue (dobel/berulang
                // bikin kesan "muter2 di situ2 aja" atau kayak loncat balik)
                var sid = s.videoId || s.id;
                return sid !== currId && !existingIds[sid];
            });
...
async function NX(){
    ...
    // FIX: pakai peekNextIndex() yang sama dgn yang dipakai preload/preview, supaya lagu yg
    // beneran diputar SELALU sama dgn yg sempat "diintip" sebelumnya - queue jadi konsisten.
    var ni = peekNextIndex();
    if(ni === -1) return;
    PK(S.ps, ni);
}
...
function renderLyricsDOM(ld) {
    ...
    html+='<p class="text-left text-[#7a4361] text-sm mt-12 mb-4 opacity-50 tracking-widest">——— end ———</p>';
    inlineHtml+='<p class="text-left text-[#7a4361] text-xs mt-8 mb-2 opacity-50 tracking-widest">——— end ———</p>';
...
function openSleepTimer() {
    ...
    if (S.sleepSecondsLeft > 0) {
        contentHtml = '<div class="text-center mb-6">' +
            '<p class="text-xs text-[rgba(255,255,255,0.4)] uppercase tracking-wider mb-1">Timer Sedang Berjalan</p>' +
        ...
        contentHtml = '<div class="text-center mb-6">' +
            '<p class="text-sm text-[#ecd0e0] font-bold mb-1">Berhenti di akhir lagu aktif</p>' +
            '<p class="text-[11px] text-[rgba(255,255,255,0.4)] mb-4">Lagu akan berhenti setelah lagu ini selesai diputar.</p>' +
        ...
        contentHtml = '<div class="grid grid-cols-3 gap-3 mb-4">' + gridHtml + '</div>' +
            '<button onclick="startSleepAtTrackEnd()" class="w-full py-3.5 px-4 rounded-2xl bg-[#ecd0e0]/10 hover:bg-[#ecd0e0]/20 border border-white/10 text-xs text-white font-bold active:scale-95 transition-all flex items-center justify-center gap-2">' +
    ...
    popup.innerHTML = '<div class="w-full max-w-md rounded-t-3xl p-6 border-t border-white/10 glass-strong" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div class="flex justify-between items-center mb-5">' +
            '<div>' +
                ...
                '<p class="text-[rgba(255,255,255,0.4)] text-xs">Hentikan musik secara otomatis saat tidur</p>' +
            '</div>' +
            '<button onclick="closeSleepTimer()" class="text-[rgba(255,255,255,0.4)] hover:text-white p-1"><i data-lucide="x" class="w-5 h-5"></i></button>' +
        '</div>' +
        contentHtml +
    '</div>';
...
function openPlaybackSpeed() {
    ...
    var optionsHtml = speeds.map(function(sp) {
        ...
        var btnStyle = isSelected 
            ? 'bg-[#ecd0e0] text-black font-bold border-[#ecd0e0]' 
    ...
    popup.innerHTML = '<div class="w-full max-w-md rounded-t-3xl p-6 border-t border-white/10 glass-strong" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color); max-height: 80vh; overflow-y: auto;">' +
        ...
        '<div class="flex justify-between items-center mb-5">' +
            '<div>' +
                ...
                '<p class="text-[rgba(255,255,255,0.4)] text-xs">Atur kecepatan putar lagu sesuai seleramu</p>' +
            '</div>' +
            '<button onclick="closePlaybackSpeed()" class="text-[rgba(255,255,255,0.4)] hover:text-white p-1"><i data-lucide="x" class="w-5 h-5"></i></button>' +
        '</div>' +
...
function openEqualizer() {
    ...
    var slidersHtml = bandsList.map(function(b, idx) {
        ...
        return '<div class="flex flex-col items-center flex-1 gap-2">' +
            '<span id="eq-val-label-' + idx + '" class="text-[10px] text-[rgba(255,255,255,0.4)] font-mono">' + (val > 0 ? '+' : '') + Math.round(val) + 'dB</span>' +
            ...
            '<span class="text-xs text-[rgba(255,255,255,0.55)] font-medium">' + b + '</span>' +
        '</div>';
    }).join('');
    ...
    var presetsHtml = presets.map(function(p) {
        ...
        var btnStyle = act ? 'bg-[#ecd0e0]/20 text-white font-bold' : 'hover:bg-white/5 text-[rgba(255,255,255,0.55)]';
    ...
    popup.innerHTML = '<div class="w-full max-w-md rounded-t-3xl p-6 border-t border-white/10 glass-strong" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        ...
        '<div class="flex justify-between items-center mb-4">' +
            '<div>' +
                ...
                '<p class="text-[rgba(255,255,255,0.4)] text-xs">Atur frekuensi suara sesuai selera</p>' +
            '</div>' +
...
function changeSlider(bandIdx, val) {
    ...
    if (pc) {
        ...
        buttons.forEach(function(btn) {
            btn.className = 'px-3.5 py-1.5 rounded-full text-xs transition-all hover:bg-white/5 text-[rgba(255,255,255,0.55)]';
        });
    }
...
function openShareCard() {
    ...
    popup.innerHTML = '<div class="w-full max-w-sm rounded-3xl p-6 border border-white/10 glass-strong text-center" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">' +
        '<div class="flex justify-between items-center mb-4">' +
            ...
            '<button onclick="document.getElementById(\'share-card-popup\').remove()" class="text-[rgba(255,255,255,0.55)] hover:text-white p-1"><i data-lucide="x" class="w-5 h-5"></i></button>' +
        '</div>' +
        ...
        '<div id="share-card-preview" class="p-6 rounded-2xl mb-6 flex flex-col items-center gap-4 relative overflow-hidden" ' +
            ...
            '<div class="w-full truncate">' +
                ...
                '<p class="text-[rgba(255,255,255,0.55)] text-xs font-bold mt-1 truncate">' + es(S.ct.artist) + '</p>' +
            '</div>' +
            ...
            '<div class="flex justify-between w-full text-[9px] text-[rgba(255,255,255,0.4)] font-mono mt-1"><span>1:48</span><span>2:56</span></div>' +
            '<div class="border-t border-white/5 w-full pt-3 mt-1 flex items-center justify-center gap-1.5">' +
                '<i data-lucide="music" class="w-3.5 h-3.5 text-[rgba(255,255,255,0.55)]"></i>' +
                '<span class="text-[10px] text-[rgba(255,255,255,0.4)] tracking-wider font-semibold uppercase">Cleo Cloud Web App</span>' +
            '</div>' +
        '</div>' +
...
function triggerNativeShare() {
    ...
    if (navigator.share) {
        navigator.share({
            title: S.ct.title,
            text: 'Dengarkan ' + S.ct.title + ' - ' + S.ct.artist + ' di Cleo Cloud!',
            url: url
        }).catch(function() {});
    } else {
        copyShareLink();
...
function downloadShareCard() {
    ...
    if (isLight) {
        grad.addColorStop(0, '#f7dcee');
        grad.addColorStop(1, '#e7c8dd');
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, 600, 800);
    } else {
        grad.addColorStop(0, '#0d0d10');
        grad.addColorStop(1, '#210a18');
        ctx.fillStyle = grad;
        ctx.fillRect(0, 0, 600, 800);
    }
    ...
    img.onload = function() {
        ...
        ctx.fillStyle = isLight ? '#4a2340' : '#ffffff';
        ctx.font = '900 32px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText(S.ct.title, 300, 540, 480);
        
        ctx.fillStyle = isLight ? '#987191' : 'rgba(255,255,255,0.55)';
        ctx.font = 'bold 24px sans-serif';
        ctx.fillText(S.ct.artist, 300, 585, 480);
        
        ctx.fillStyle = isLight ? '#cf9fc0' : '#7a4361';
        ctx.font = '16px monospace';
        ctx.fillText('DIDENGARKAN DI CLEO CLOUD', 300, 710);
        
        try {
            var dataUrl = canvas.toDataURL('image/png');
            var a = document.createElement('a');
            a.download = S.ct.title.replace(/[^a-zA-Z0-9]/g, '_') + '_cleocloud.png';
            a.href = dataUrl;
            a.click();
...
function openQueue(){
    ...
    if(!S.pl||S.pl.length===0){
        listHtml='<div class="text-center text-[rgba(255,255,255,0.4)] py-10"><i data-lucide="list-music" class="w-12 h-12 mx-auto mb-3 opacity-30"></i><p class="text-sm">Antrian kosong</p></div>';
    }else{
        listHtml=S.pl.map(function(t,i){
            ...
            return '<div onclick="playQueueIndex('+i+')" class="flex items-center gap-3 p-2.5 rounded-xl cursor-pointer active:scale-[0.98] '+(active?'bg-white/10':'hover:bg-white/5')+'">'+
                ...
                '<div class="flex-1 truncate"><p class="text-sm font-medium truncate '+(active?'text-[#ecd0e0]':'text-white')+'">'+es(t.title)+'</p><p class="text-[rgba(255,255,255,0.4)] text-xs truncate">'+es(t.artist)+'</p></div>'+
                (active?'<i data-lucide="volume-2" class="w-4 h-4 text-[#ecd0e0] flex-shrink-0"></i>':'<span class="text-[rgba(255,255,255,0.4)] text-xs flex-shrink-0">'+(i+1)+'</span>')+
            '</div>';
        }).join('');
    }
    ...
    popup.innerHTML='<div class="w-full max-w-md rounded-t-3xl p-6 border-t border-white/10 glass-strong" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color); max-height:75vh; display:flex; flex-direction:column;">'+
        ...
        '<div class="flex justify-between items-center mb-4 flex-shrink-0">'+
            '<div><h3 class="font-black text-white text-lg">Daftar Antrian</h3><p class="text-[rgba(255,255,255,0.4)] text-xs">'+(S.pl?S.pl.length:0)+' lagu dalam antrian</p></div>'+
            '<button onclick="closeQueue()" class="text-[rgba(255,255,255,0.4)] hover:text-white p-1"><i data-lucide="x" class="w-5 h-5"></i></button>'+
        '</div>'+
