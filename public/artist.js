var Artist={
    init(){
        ...
        <div id="artist-modal" class="fixed inset-0 bg-[#0d0d10] flex flex-col z-30" style="display:none; animation: slideUp 0.3s ease-out forwards;">
            ...
            <div class="flex-1 overflow-y-auto hide-scrollbar pb-36 relative" id="artist-content" onscroll="Artist.handleScroll()">
                <p class="text-center text-[rgba(255,255,255,0.55)] mt-32">Memuat...</p>
            </div>
        </div>`;
        lucide.createIcons();
    },
    ...
    open(id, name, thumbnail) {
        ...
        <div class="relative w-full aspect-square max-h-[40vh] overflow-hidden -mt-16 mb-6">
            <img loading="lazy" decoding="async" src="${tImg}" class="w-full h-full object-cover opacity-50" onerror="this.src='${FI}'" />
            <div class="absolute inset-0 bg-gradient-to-t from-[#270d1e] to-transparent"></div>
            <div class="absolute bottom-6 left-6 right-6 flex flex-col justify-end items-center text-center">
                <img loading="lazy" decoding="async" src="${tImg}" class="w-24 h-24 rounded-full mb-3 object-cover" onerror="this.src='${FI}'" />
                <h2 class="text-2xl font-black text-white drop-shadow-md">${es(name || 'Artist')}</h2>
            </div>
        </div>
        <div id="artist-inner-content">
            <div class="flex justify-center mt-12">
                <div class="w-8 h-8 border-3 border-[#ecd0e0] border-t-transparent rounded-full animate-spin"></div>
            </div>
        </div>`;
        ...
        fetch(API.artist + '?id=' + id).then(function(r){return r.json();}).then(function(d){
            if(d.status && d.result){
                ...
                <div class="relative w-full aspect-square max-h-[50vh] overflow-hidden -mt-20 mb-6">
                    <img loading="lazy" decoding="async" src="${headerImg}" class="w-full h-full object-cover" onerror="this.src='${FI}'" />
                    <div class="absolute inset-0 bg-gradient-to-t from-[#270d1e] via-[#270d1e]/60 to-transparent"></div>
                    <div class="absolute bottom-6 left-6 right-6 flex flex-col justify-end items-center text-center">
                        <img loading="lazy" decoding="async" src="${headerImg}" class="artist-photo mb-4" onerror="this.src='${FI}'" />
                ...
                if(a.topSongs&&a.topSongs.length>0){
                    ...
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[rgba(255,255,255,0.55)] uppercase tracking-wider mb-3 px-4">Lagu Teratas</h3><div id="artist-songs-list" class="space-y-1 px-2">';
                    a.topSongs.slice(0,10).forEach(function(s,i){
                        ...
                        var rowBg = isPlay ? 'bg-[rgba(255,255,255,0.14)] border border-white/40 shadow-xl' : (isCur ? 'bg-[rgba(255,255,255,0.12)] border border-white/30' : 'bg-[rgba(255,255,255,0.06)] border border-white/10 hover:bg-[rgba(255,255,255,0.1)]');
                        ...
                        <div onclick="Artist.play('${s.videoId}','${esJs(s.title)}','${esJs(s.artist||a.name)}','${esJs(im)}')" class="flex items-center gap-3 p-2.5 rounded-2xl cursor-pointer active:scale-[0.98] transition-all duration-300 shadow-lg group ${rowBg}">
                            ...
                            <img loading="lazy" decoding="async" src="${im}" class="w-12 h-12 rounded-xl object-cover shadow-md shrink-0 border border-white/10" onerror="this.src='${FI}'" />
                ...
                if(a.topAlbums&&a.topAlbums.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[rgba(255,255,255,0.55)] uppercase tracking-wider mb-3 px-4">Album</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.topAlbums.forEach(function(al){
                        ...
                        html+=`
                        <div onclick="Album.open('${al.browseId}', '${esJs(im)}')" class="flex-shrink-0 w-36 cursor-pointer group p-2 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all">
                            <div class="w-full aspect-square rounded-xl overflow-hidden mb-2 shadow-md">
                                <img loading="lazy" decoding="async" src="${im}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src='${FI}'" />
                            </div>
                ...
                if(a.topSingles&&a.topSingles.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[rgba(255,255,255,0.55)] uppercase tracking-wider mb-3 px-4">Singles & EP</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.topSingles.forEach(function(sg){
                        ...
                        html+=`
                        <div onclick="Album.open('${sg.browseId}', '${esJs(im)}')" class="flex-shrink-0 w-36 cursor-pointer group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col">
                            <div class="w-full aspect-square rounded-xl overflow-hidden mb-2 shadow-md">
                                <img loading="lazy" decoding="async" src="${im}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src='${FI}'" />
                            </div>
                ...
                if(a.topVideos&&a.topVideos.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[rgba(255,255,255,0.55)] uppercase tracking-wider mb-3 px-4">Video</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.topVideos.forEach(function(vd){
                        ...
                        html+=`
                        <div onclick="Artist.play('${vd.videoId||''}','${esJs(vd.name)}','${esJs(vd.artist||a.name)}','${esJs(im)}')" class="flex-shrink-0 w-48 cursor-pointer group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col">
                ...
                if(a.playlists&&a.playlists.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[rgba(255,255,255,0.55)] uppercase tracking-wider mb-3 px-4">Playlist</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.playlists.forEach(function(pl){
                        ...
                        html+=`
                        <div onclick="Album.open('${pl.browseId}', '${esJs(im)}')" class="flex-shrink-0 w-36 cursor-pointer group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col">
                            <div class="w-full aspect-square rounded-xl overflow-hidden mb-2 shadow-md">
                                <img loading="lazy" decoding="async" src="${im}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src='${FI}'" />
                            </div>
                ...
                if(a.featuredOn&&a.featuredOn.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[rgba(255,255,255,0.55)] uppercase tracking-wider mb-3 px-4">Tampil Di</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.featuredOn.forEach(function(fo){
                        ...
                        html+=`
                        <div onclick="Album.open('${fo.browseId}', '${esJs(im)}')" class="flex-shrink-0 w-36 cursor-pointer group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col">
                            <div class="w-full aspect-square rounded-xl overflow-hidden mb-2 shadow-md">
                                <img loading="lazy" decoding="async" src="${im}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src='${FI}'" />
                            </div>
                ...
                if(a.similarArtists&&a.similarArtists.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[rgba(255,255,255,0.55)] uppercase tracking-wider mb-3 px-4">Artis Serupa</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.similarArtists.forEach(function(s){
                        ...
                        html+=`
                        <div onclick="Artist.open('${s.browseId}','${esJs(s.name)}')" class="flex-shrink-0 w-32 cursor-pointer group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col items-center">
                            <div class="w-20 h-20 rounded-full overflow-hidden mb-2 border-2 border-white/10 shadow-md group-hover:scale-105 transition-transform duration-300">
                                <img loading="lazy" decoding="async" src="${im}" class="w-full h-full object-cover" onerror="this.src='${FI}'" />
                            </div>
    ...
    share() {
        ...
        if (navigator.share) {
            navigator.share({
                title: name + ' - Cleo Cloud',
                text: 'Dengarkan lagu & album terbaik dari ' + name + ' di Cleo Cloud!',
                url: url
            }).catch(function() {});
        } else {
