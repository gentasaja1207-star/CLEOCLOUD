var Search={
    render(){
        ...
        <div id="filter-tabs" class="hidden flex gap-2 p-1.5 glass rounded-full mx-4 mb-3 mt-4 border border-white/10">
            ...
            <button onclick="setFilter('playlists')" id="f-playlists" class="filter-tab flex-1 py-2 px-4 rounded-full text-xs font-semibold text-[rgba(255,255,255,0.55)] hover:text-white transition-all">Playlist</button>
            <button onclick="setFilter('artists')" id="f-artists" class="filter-tab flex-1 py-2 px-4 rounded-full text-xs font-semibold text-[rgba(255,255,255,0.55)] hover:text-white transition-all">Artis</button>
        </div>
    ...
    showRecs(){
        ...
        rc.innerHTML=Search.REC_ROWS.map(function(row){
            ...
            var cardsHtml=list.map(function(t,i){
                ...
                return '<div onclick="PK(\''+row.key+'\','+i+')" class="search-rec-item flex-shrink-0 w-36 cursor-pointer active:scale-95 group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col animate-card-left" style="animation-delay:'+Math.min(i*50, 450)+'ms"><div class="search-rec-cover w-full aspect-square mb-2 relative rounded-xl overflow-hidden shadow-md transition-all '+ringClass+'"><img loading="lazy" decoding="async" src="'+t.cover+'" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" onerror="this.src=\''+FI+'\'" /><div class="search-rec-btn">'+recBtn+'</div></div><h3 class="search-rec-title font-semibold text-xs truncate text-white px-0.5 '+(isCur?'font-black':'')+'">'+es(t.title)+'</h3><p class="text-white/60 text-[10px] truncate mt-0.5 px-0.5">'+es(t.artist)+'</p></div>';
            }).join('');
    ...
    renderActive(){
        ...
        if (c && S.sq && S.filter === 'songs' && S.sr) {
            ...
            items.forEach(function(el, i){
                ...
                var itemBg = isPlay ? 'bg-[rgba(255,255,255,0.14)] border border-white/40 shadow-xl' : (isCur ? 'bg-[rgba(255,255,255,0.12)] border border-white/30' : 'bg-[rgba(255,255,255,0.06)] border border-white/10 hover:bg-[rgba(255,255,255,0.1)]');
    ...
    updateFilterUI(){
        document.querySelectorAll('.filter-tab').forEach(function(el){
            el.className = 'filter-tab flex-1 py-2 px-4 rounded-full text-xs font-semibold text-[rgba(255,255,255,0.55)] hover:text-white transition-all';
        });
    ...
    show(loading){
        ...
        if(loading){c.innerHTML='<div class="text-center mt-10"><div class="w-8 h-8 border-3 border-[#ecd0e0] border-t-transparent rounded-full animate-spin mx-auto"></div></div>';return;}
        ...
        if (S.filter === 'songs') {
            c.innerHTML=S.sr.map(function(t,i){
                ...
                return '<div onclick="PK(\'search\','+i+')" class="search-song-item flex items-center gap-3.5 p-2.5 mb-2 rounded-2xl cursor-pointer active:scale-[0.98] transition-all shadow-lg shadow-black/25 animate-card-up '+itemBg+'" style="animation-delay:'+Math.min(i*35, 500)+'ms">'+
                    '<img loading="lazy" decoding="async" src="'+toWebp(t.cover)+'" class="w-12 h-12 rounded-xl object-cover shadow-md shrink-0 border border-white/10" onerror="handleImgError(this)" />'+
            ...
            c.innerHTML='<div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 pb-8">'+S.sr.map(function(p, i){
                return '<div onclick="Artist.open(\''+p.id+'\', \''+esJs(p.name||p.title)+'\')" class="p-3 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] cursor-pointer active:scale-95 transition-all text-center flex flex-col items-center justify-center group animate-card-up" style="animation-delay:'+Math.min(i*40, 500)+'ms"><div class="relative w-20 h-20 mb-2.5 rounded-full overflow-hidden border-2 border-white/10 shadow-md group-hover:scale-105 transition-transform duration-300"><img loading="lazy" decoding="async" src="'+toWebp(p.cover)+'" class="w-full h-full object-cover" onerror="handleImgError(this)" /></div><h3 class="font-bold text-sm truncate text-white w-full px-1">'+es(p.name||p.title)+'</h3><p class="text-white/60 text-[10px] uppercase tracking-wider font-semibold mt-0.5">'+es(p.subtitle||'Artist')+'</p></div>';
            }).join('')+'</div>';
            lucide.createIcons();
        } else {
            c.innerHTML='<div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 pb-8">'+S.sr.map(function(p, i){
                return '<div onclick="Album.open(\''+p.id+'\', \''+(p.cover||FI)+'\')" class="p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] cursor-pointer active:scale-95 transition-all group flex flex-col animate-card-up" style="animation-delay:'+Math.min(i*40, 500)+'ms"><div class="w-full aspect-square mb-2.5 rounded-xl overflow-hidden shadow-md"><img loading="lazy" decoding="async" src="'+toWebp(p.cover)+'" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="handleImgError(this)" /></div><h3 class="font-semibold text-sm truncate text-white px-0.5">'+es(p.title)+'</h3><p class="text-white/60 text-xs truncate mt-0.5 px-0.5">'+es(p.artist)+'</p></div>';
            }).join('')+'</div>';
            lucide.createIcons();
        }
    },
