...
window.addEventListener('beforeinstallprompt',function(e){
    ...
    // Browser cuma nge-fire event ini kalau app BELUM terinstall di device.
    // Jadi ini sinyal paling akurat buat reset flag lama (misal user abis hapus APK-nya).
...
window.addEventListener('appinstalled',function(){
    ...
    showToast('Cleo Cloud berhasil diinstall!');
});
...
function showPwaRequiredModal() {
    ...
    modal.innerHTML = '<div class="bg-[#240b1c] border border-white/15 rounded-2xl p-5 max-w-xs w-full text-center space-y-3 shadow-2xl relative" onclick="event.stopPropagation()">'+
        ...
        '<div class="space-y-1">'+
            ...
            '<p class="text-white/60 text-xs leading-relaxed">'+
                'Fitur Mode Offline khusus untuk aplikasi PWA. Silakan install Cleo Cloud ke layar utama terlebih dahulu.'+
            '</p>'+
        '</div>'+
    ...
    if(deferredInstallPrompt){
        ...
        deferredInstallPrompt.userChoice.then(function(choice){
            if(choice.outcome==='accepted') {
                try { localStorage.setItem('pwa_installed', 'true'); } catch(e){}
                isStandaloneApp = true;
                showToast('Menginstall Cleo Cloud...');
            }
            deferredInstallPrompt=null;
            var btn=document.getElementById('pwa-install-btn');
            if(btn)btn.classList.add('hidden');
...
async function saveTrackForOffline(track) {
    ...
    if (existingIndex !== -1) {
        ...
        // Hapus juga file audio yang tersimpan di Cache Storage biar nggak makan storage percuma
        try {
            if (typeof audioUrlCache !== 'undefined' && audioUrlCache[vid] && 'caches' in window) {
                var delProxyUrl = '/api/proxy-audio?url=' + encodeURIComponent(audioUrlCache[vid]);
                caches.open('cleocloud-api-v3').then(function(c){ c.delete(delProxyUrl); });
            }
        } catch(e) {}
    ...
    // 2. Pre-fetch Audio URL (kalau belum ada di cache)
    try {
    ...
    // 2b. Benar-benar download & simpan file audio-nya ke Cache Storage biar bisa diputar offline
    try {
        if (typeof audioUrlCache !== 'undefined' && audioUrlCache[vid] && 'caches' in window) {
            var proxyUrl = '/api/proxy-audio?url=' + encodeURIComponent(audioUrlCache[vid]);
            var audioRes = await fetch(proxyUrl);
            if (audioRes && audioRes.ok) {
                var cache = await caches.open('cleocloud-api-v3');
                await cache.put(proxyUrl, audioRes.clone());
                songObj.audioCached = true;
            }
        }
    } catch(e) {}
...
// Simpan SEMUA lagu dalam satu playlist ke Mode Offline PWA sekaligus
var offlineBulkBusy = false;
async function savePlaylistForOffline(plId) {
    if (!isPwaInstalled()) {
        showPwaRequiredModal();
        return;
    }
    if (offlineBulkBusy) {
        showToast('Sedang menyimpan playlist lain, tunggu sebentar...');
        return;
    }
    ...
    if (!pl || !pl.songs || !pl.songs.length) {
        showToast('Playlist ini kosong');
        return;
    }

    var songsToSave = pl.songs.filter(function(s) { return !isOfflineSong(s); });
    if (songsToSave.length === 0) {
        showToast('Semua lagu di playlist ini sudah tersimpan offline');
        return;
    }

    offlineBulkBusy = true;
    var btn = gid('offline-save-all-btn');
    var total = songsToSave.length;
    var done = 0;

    function setBtnProgress() {
        if (!btn) return;
        btn.disabled = true;
        btn.innerHTML = '<div class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div><span>' + done + '/' + total + '</span>';
    }
    setBtnProgress();
    showToast('Menyimpan ' + total + ' lagu untuk offline...');

    for (var i = 0; i < songsToSave.length; i++) {
        var s = songsToSave[i];
        var list = getOfflineSongs();
        var already = list.some(function(x) { return (x.videoId === (s.videoId || s.id) || x.id === (s.videoId || s.id)); });
        if (!already) {
            await saveTrackForOfflineSilent(s);
        }
        done++;
        setBtnProgress();
    }

    offlineBulkBusy = false;
    showToast('Selesai! ' + total + ' lagu tersimpan untuk offline');
    if (typeof Library !== 'undefined' && Library.currentPlaylistId === plId) Library.open(plId);
}

// Versi saveTrackForOffline yang tidak toggle-off kalau sudah tersimpan (khusus dipanggil dari bulk save)
async function saveTrackForOfflineSilent(track) {
    if (!track) return;
    var vid = track.videoId || track.id;
    if (!vid) return;
    if (isOfflineSong(track)) return;
    ...
    try {
        ...
        var ld = await lr.json();
        if (ld && ld.status && ld.result && ld.result.lyrics) {
            var cachedLyric = { vid: vid, type: ld.result.lyrics.type || 'none', lines: ld.result.lyrics.lines || [] };
...
// Cek apakah semua lagu di sebuah playlist sudah tersimpan offline (buat status tombol)
function isPlaylistFullyOffline(plId) {
    var pls = typeof getUserPlaylists === 'function' ? getUserPlaylists() : [];
    var pl = pls.find(function(p) { return p.id === plId; });
    if (!pl || !pl.songs || !pl.songs.length) return false;
    return pl.songs.every(function(s) { return isOfflineSong(s); });
}
            ...
            songsHtml = offlineSongs.map(function(s, i) {
                ...
                return '<div class="flex items-center gap-3 p-2.5 rounded-2xl border '+cardBg+' active:scale-[0.98] transition-all duration-200 group shadow-md">'+
                    '<div onclick="PK(\'offline\','+i+')" class="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">'+
                        ...
                        '<img loading="lazy" decoding="async" src="'+(s.cover || FI)+'" class="w-12 h-12 rounded-xl object-cover shrink-0 shadow-md border border-white/10" onerror="this.src=\''+FI+'\'" />'+
...
var App={
    ...
    autoPlayTrack(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='Cleo Cloud',cover=toHDCover('', videoId),artistId='';
            ...
            S.ct={id:videoId,videoId:videoId,title:'Lagu',artist:'Cleo Cloud',cover:toHDCover('', videoId),artistId:'',ytUrl:'https://youtube.com/watch?v='+videoId};
            S.ps='direct';S.pl=[S.ct];S.pi=0;UU();MP.show();resetLyricsUI(videoId);
            FullPlayer.open();loadTrack(S.ct);
        });
    },
    showSharePopup(videoId){
        fetch(API.search+'?query=https://youtube.com/watch?v='+videoId).then(function(r){return r.json();}).then(function(d){
            var title='Lagu',artist='Cleo Cloud',cover=toHDCover('', videoId);
        ...
        }).catch(function(){App.renderPopup(videoId,'Lagu','Cleo Cloud',toHDCover('', videoId));});
    },
                ...
                <div class="space-y-4 text-left mb-6 max-h-[250px] overflow-y-auto pr-1">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[rgba(255,255,255,0.55)] text-xs leading-relaxed">Sesuaikan Bass, Mid, Treble, dan gunakan berbagai Preset Keren untuk kualitas audio musik terbaik.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[rgba(255,255,255,0.55)] text-xs leading-relaxed">Bagikan lagu favorit Anda menggunakan link audio langsung untuk kemudahan berbagi musik.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[rgba(255,255,255,0.55)] text-xs leading-relaxed">Atur waktu putar musik otomatis sebelum tidur dengan durasi yang dapat ditentukan sendiri.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[rgba(255,255,255,0.55)] text-xs leading-relaxed">Dilengkapi opsi agar lagu aktif Anda tetap berputar sampai selesai sebelum pemutaran otomatis berhenti tanpa memotong lagu di tengah-tengah.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[rgba(255,255,255,0.55)] text-xs leading-relaxed">Memungkinkan Anda mempercepat atau memperlambat musik sesuai kebutuhan (mendukung kecepatan 0.5x, 0.75x, 1.0x (Normal), 1.25x, 1.5x, 1.75x, hingga 2.0x).</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[rgba(255,255,255,0.55)] text-xs leading-relaxed">Kustomisasi getaran audio dengan mengubah kecepatan musik secara instan ke gaya favorit Anda.</p>
                        </div>
                    </div>
                </div>
