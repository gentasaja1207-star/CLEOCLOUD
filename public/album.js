var Album = {
    init() {
        ...
        <div id="album-modal" class="fixed inset-0 bg-[#050506] flex flex-col z-[100]" style="display:none; animation: slideUp 0.3s ease-out forwards;">
            ...
            <div class="flex-1 overflow-y-auto hide-scrollbar pb-36 relative" id="album-content" onscroll="Album.handleScroll()">
                <div class="flex justify-center mt-32">
                    <div class="w-10 h-10 border-3 border-[#ecd0e0] border-t-transparent rounded-full animate-spin"></div>
                </div>
            </div>
        </div>`;
        lucide.createIcons();
    ...
    open(id, passedCoverUrl) {
        ...
        .then(data => {
            ...
            <div class="relative w-full aspect-square md:aspect-video max-h-[50vh] overflow-hidden -mt-20 mb-6">
                <img loading="lazy" decoding="async" src="${im}" class="w-full h-full object-cover" />
                ...
                <div class="absolute bottom-6 left-6 right-6 flex flex-col justify-end items-center text-center z-10">
                    <img loading="lazy" decoding="async" src="${im}" class="w-32 h-32 md:w-48 md:h-48 rounded-xl object-cover border border-white/10 mb-4" onerror="this.src='${FI}'" />
                    <div>
            ...
            if(a.songs && a.songs.length > 0) {
                ...
                a.songs.forEach((s, i) => {
                    ...
                    var rowBg = isPlay ? 'bg-[rgba(255,255,255,0.14)] border border-white/40 shadow-xl' : (isCur ? 'bg-[rgba(255,255,255,0.12)] border border-white/30' : 'backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 hover:bg-[rgba(255,255,255,0.1)]');
                    ...
                    <div onclick="Album.playSong('${id}', ${i})" class="flex items-center gap-3 p-2.5 rounded-2xl cursor-pointer group active:scale-[0.98] transition-all duration-300 shadow-lg ${rowBg}">
                        ...
                        <img loading="lazy" decoding="async" src="${sim}" class="w-12 h-12 rounded-xl object-cover shadow-md shrink-0 border border-white/10" onerror="this.src='${FI}'" />
    ...
    share(id) {
        ...
        if (navigator.share) {
            navigator.share({
                title: title + ' - Cleo Cloud',
                text: 'Dengarkan album ' + title + ' di Cleo Cloud!',
                url: url
            }).catch(function() {});
        } else {
