var MP={
    init(){
        ...
        <div id="mini-player" class="hidden fixed left-4 right-4 sm:left-auto sm:right-6 sm:w-96 z-[160]" style="bottom:82px;transition:transform 0.35s cubic-bezier(0.175, 0.885, 0.32, 1.275);will-change:transform;transform:translate3d(0,150px,0);">
            <div id="mini-player-inner" onclick="FullPlayer.open()" class="rounded-2xl px-3.5 py-2.5 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition-all relative overflow-hidden bg-white/[0.12] backdrop-blur-2xl border border-white/20 shadow-2xl group">
                ...
                <div id="mini-next-overlay" class="absolute inset-0 pointer-events-none opacity-0 z-20 flex items-center gap-3 px-3.5 py-2.5 rounded-2xl bg-[#240b1c]/95 border border-white/30 backdrop-blur-2xl overflow-hidden">
                    <div class="relative w-11 h-11 shrink-0 flex items-center justify-center">
                        <div class="w-[34px] h-[34px] rounded-full overflow-hidden border border-white/20">
                            <img loading="lazy" decoding="async" id="mini-cover-next" src="" class="w-full h-full object-cover rounded-full" />
                        </div>
                    </div>
                ...
                <div class="relative w-11 h-11 shrink-0 flex items-center justify-center z-10" onclick="FullPlayer.open(); if(typeof event !== 'undefined') event.stopPropagation();">
                    <svg class="w-11 h-11 -rotate-90 pointer-events-none absolute inset-0 z-10" viewBox="0 0 48 48">
                        ...
                        <circle id="mini-circle-progress" cx="24" cy="24" r="21" fill="none" stroke="#ec4899" stroke-width="2.5" stroke-dasharray="131.95" stroke-dashoffset="131.95" stroke-linecap="round" class="transition-all duration-150"></circle>
                    </svg>
                    <div class="w-[34px] h-[34px] rounded-full overflow-hidden  z-0 border border-white/10">
                        <img loading="lazy" decoding="async" id="mini-cover" src="" class="w-full h-full object-cover rounded-full" />
                    </div>
                </div>
                ...
                <div class="flex-1 min-w-0 z-10">
                    ...
                    <div id="mini-artist" class="text-[rgba(255,255,255,0.55)] text-[11px] truncate mt-0.5"></div>
                </div>
                ...
                <div class="flex items-center gap-1.5 z-10 shrink-0">
                    ...
                    <button id="mini-like-btn" onclick="toggleCurrentLike(); if(typeof event !== 'undefined') event.stopPropagation();" class="text-[rgba(255,255,255,0.55)] hover:text-rose-400 active:scale-90 p-1.5 cursor-pointer" title="Sukai Lagu">
    ...
    getTrackColors(track) {
        if (!track) return ['#ff2d95', '#c026d3', '#db2777', '#f472b6', '#e11d48'];
    ...
    applyColors(colors) {
        ...
        if (inner) inner.style.background = "linear-gradient(to right, " + "color-mix(in srgb, " + tertiary + " 30%, #240b1c), #240b1c 80%)";
