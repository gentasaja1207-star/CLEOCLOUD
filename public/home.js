var Home = {
    ...
    render() {
        var chipsHtml = Home.categories.map(function(c) {
            ...
            var btnStyle = isActive
                ? 'bg-white text-black font-bold shadow-lg shadow-black/20 border border-white scale-105'
                : 'bg-white/[0.06] backdrop-blur-xl text-white/70 hover:text-white hover:bg-white/[0.1] border border-white/[0.12] font-medium';
            return '<button onclick="Home.selectCategory(\'' + c.name + '\')" class="home-chip-btn px-4 py-2 rounded-full text-xs whitespace-nowrap transition-transform duration-300 flex items-center gap-1.5 cursor-pointer shrink-0 active:scale-95 ' + btnStyle + '">' +
        ...
        <div class="pt-8 pb-3.5 px-4 sticky top-0 z-30 border-b border-white/[0.08] transition-all cc-header-blur">
            <div class="flex justify-between items-center mb-3">
                <div>
                    <h1 class="text-3xl font-black text-white tracking-tight drop-shadow-md">Cleo Cloud</h1>
                </div>
        ...
        if (catView) {
            ...
            <div class="mb-4 flex justify-between items-center bg-white/5 p-3.5 rounded-2xl border border-white/10 animate-pulse">
                <div class="flex items-center gap-2">
                    <span class="text-xs text-[rgba(255,255,255,0.55)]">Kategori:</span>
                    <span class="font-bold text-sm text-white">${es(catName)}</span>
                </div>
                <button onclick="Home.selectCategory('Semua')" class="text-xs px-3 py-1.5 rounded-full bg-white/10 hover:bg-white/20 text-[rgba(255,255,255,0.55)] hover:text-white transition-all flex items-center gap-1">
            ...
            <div class="text-center py-12">
                ...
                <p class="text-xs text-[rgba(255,255,255,0.55)] animate-pulse">Memuat musik ${es(catName)}...</p>
            </div>`;
            lucide.createIcons();
        }
    ...
    renderDeveloperProfileView() {
        ...
        if (S.hc && S.hc.length > 0) {
            songsHtml = S.hc.map(function(t, i) {
                ...
                return '<div onclick="PK(\'homecat\','+i+')" class="snap-start home-cat-card group '+ (isPlay ? 'bg-[rgba(255,255,255,0.14)] border border-white/40 shadow-xl' : (isCur ? 'bg-[rgba(255,255,255,0.12)] border border-white/30' : 'bg-[rgba(255,255,255,0.06)] border border-white/10 hover:bg-[rgba(255,255,255,0.1)]')) +' rounded-2xl flex items-center gap-3 p-2.5 cursor-pointer active:scale-95 transition-all w-full shadow-lg shadow-black/20">'+
                    '<img loading="lazy" decoding="async" src="'+t.cover+'" class="w-12 h-12 rounded-xl object-cover shadow-md shrink-0 border border-white/10" referrerPolicy="no-referrer" onerror="this.src=\''+FI+'\'" />'+
        ...
        if (S.hcp && S.hcp.length > 0) {
            plistHtml = S.hcp.map(function(p, i) {
                return '<div onclick="Album.open(\''+p.id+'\', \''+(p.cover||FI)+'\')" class="flex-shrink-0 w-36 cursor-pointer active:scale-95 group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col"><div class="w-full aspect-square mb-2 relative rounded-xl overflow-hidden shadow-md"><img loading="lazy" decoding="async" src="'+(p.cover||FI)+'" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" referrerPolicy="no-referrer" onerror="this.src=\''+FI+'\'" /></div><h3 class="font-semibold text-xs truncate text-white px-0.5">'+es(p.title)+'</h3><p class="text-white/60 text-[10px] truncate mt-0.5 px-0.5">'+es(p.artist)+'</p></div>';
            }).join('');
        }
            ...
            <div class="glass-strong rounded-3xl p-5 border border-white/15 bg-gradient-to-br from-white/10 via-white/5 to-transparent relative overflow-hidden ">
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left relative z-10">
                    <div class="relative w-20 h-20 rounded-2xl overflow-hidden border-2 border-white/20  shrink-0 glass">
                        <img loading="lazy" decoding="async" src="/logo.png" class="w-full h-full object-cover" onerror="this.src='${FI}'" />
                    </div>
                    <div class="flex-1">
                        ...
                        <h2 class="text-2xl font-black text-white">Cleo Cloud</h2>
                        <p class="text-xs text-white/40 mt-1 tracking-wide">Created &amp; Developed by</p>
                        <p class="text-sm font-bold text-white mt-0.5">Genta De Carlo</p>
                        <div class="flex flex-wrap items-center gap-2 mt-3.5 justify-center sm:justify-start">
                            <a href="https://whatsapp.com/channel/0029Vb93nxsHltYDfZozNZ2Q" target="_blank" class="btn-chrome px-3.5 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-md active:scale-95 transition-all">
                            ...
                            <button onclick="Home.selectCategory('Semua')" class="glass hover:bg-white/10 px-3 py-1.5 rounded-full text-xs text-[rgba(255,255,255,0.55)] hover:text-white flex items-center gap-1">
                                <i data-lucide="x" class="w-3.5 h-3.5"></i> Reset
                            </button>
                        </div>
                    </div>
    ...
    displayCategoryView() {
        ...
        if (S.hc && S.hc.length > 0) {
            songsHtml = S.hc.map(function(t, i) {
                ...
                return '<div onclick="PK(\'homecat\','+i+')" class="home-cat-card group '+ (isPlay ? 'bg-[rgba(255,255,255,0.14)] border border-white/40 shadow-xl' : (isCur ? 'bg-[rgba(255,255,255,0.12)] border border-white/30' : 'bg-[rgba(255,255,255,0.06)] border border-white/10 hover:bg-[rgba(255,255,255,0.1)]')) +' rounded-2xl flex items-center gap-3 p-2.5 cursor-pointer active:scale-95 transition-all animate-card-up shadow-lg shadow-black/20" style="animation-delay:'+Math.min(i*30, 450)+'ms">'+
                    '<img loading="lazy" decoding="async" src="'+t.cover+'" class="w-12 h-12 rounded-xl object-cover shadow-md shrink-0 border border-white/10" onerror="this.src=\''+FI+'\'" />'+
        ...
        if (S.hcp && S.hcp.length > 0) {
            plistHtml = S.hcp.slice(0, 10).map(function(p, i) {
                return '<div onclick="Album.open(\''+p.id+'\', \''+(p.cover||FI)+'\')" class="flex-shrink-0 w-36 cursor-pointer active:scale-95 group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col animate-card-left" style="animation-delay:'+Math.min(i*40, 400)+'ms"><div class="w-full aspect-square mb-2 relative rounded-xl overflow-hidden shadow-md"><img loading="lazy" decoding="async" src="'+(p.cover||FI)+'" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src=\''+FI+'\'" /></div><h3 class="font-semibold text-xs truncate text-white px-0.5">'+es(p.title)+'</h3><p class="text-white/60 text-[10px] truncate mt-0.5 px-0.5">'+es(p.artist)+'</p></div>';
            }).join('');
        }
        ...
        if (S.hca && S.hca.length > 0) {
            artistsHtml = S.hca.slice(0, 8).map(function(p, i) {
                return '<div onclick="Artist.open(\''+p.id+'\', \''+esJs(p.name||p.title)+'\')" class="flex-shrink-0 w-32 cursor-pointer active:scale-95 group p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col items-center animate-card-left" style="animation-delay:'+Math.min(i*40, 400)+'ms"><div class="w-24 h-24 mb-2 relative rounded-full overflow-hidden shadow-md border-2 border-white/10 group-hover:scale-105 transition-transform duration-300"><img loading="lazy" decoding="async" src="'+(p.cover||FI)+'" class="w-full h-full object-cover" onerror="this.src=\''+FI+'\'" /></div><h3 class="font-semibold text-center text-xs truncate text-white w-full px-0.5">'+es(p.name||p.title)+'</h3><p class="text-white/60 text-[10px] uppercase tracking-wider font-semibold mt-0.5">Artist</p></div>';
            }).join('');
        }
    ...
    show() {
        ...
        if ((!S.ht || S.ht.length === 0) && !navigator.onLine) {
            g.innerHTML = '<div class="col-span-2 md:col-span-4 bg-[rgba(255,255,255,0.06)] border border-white/10 rounded-2xl p-5 text-center space-y-2 shadow-xl">'+
            ...
            g.innerHTML = (S.ht || []).slice(0, 6).map(function(t, i) {
                ...
                var cardBg = isPlay ? 'bg-[rgba(255,255,255,0.14)] border border-white/40 shadow-xl' : (isCur ? 'bg-[rgba(255,255,255,0.12)] border border-white/30' : 'bg-[rgba(255,255,255,0.06)] border border-white/10 hover:bg-[rgba(255,255,255,0.1)]');
                ...
                return '<div onclick="PK(\'home1\','+i+')" class="home-grid-card '+cardBg+' rounded-2xl p-2.5 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition-all duration-300 group shadow-lg shadow-black/25">'+
                    '<img loading="lazy" decoding="async" src="'+t.cover+'" class="w-14 h-14 rounded-xl object-cover shrink-0 shadow-md border border-white/10" onerror="this.src=\''+FI+'\'" />'+
        ...
        pls.forEach(function(p, i) {
            plHtml += '<div onclick="Library.open(\''+p.id+'\')" class="flex-shrink-0 w-36 md:w-44 cursor-pointer active:scale-95 transition-all group"><div class="p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl group-hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col"><div class="w-full aspect-square rounded-xl overflow-hidden relative shadow-md mb-2"><img loading="lazy" decoding="async" src="'+(p.image||(p.songs.length>0?p.songs[0].cover:FI))+'" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.src=\''+FI+'\'" /><div class="absolute bottom-2 right-2 bg-white/20 backdrop-blur-md border border-white/30 rounded-full p-2.5 opacity-0 group-hover:opacity-100 transition-all shadow-black/40"><i data-lucide="play" class="w-4 h-4 text-white fill-current ml-0.5"></i></div></div><h3 class="font-semibold text-sm text-white truncate px-0.5">'+es(p.name)+'</h3><p class="text-xs text-white/60 truncate mt-0.5 px-0.5">'+p.songs.length+' lagu</p></div></div>';
        });

        plHtml += '<div onclick="if(typeof Library !== \'undefined\') Library.createNew()" class="flex-shrink-0 w-36 md:w-44 cursor-pointer active:scale-95 transition-all group"><div class="p-2.5 rounded-2xl bg-white/[0.03] border border-dashed border-white/20 shadow-xl group-hover:border-white/40 group-hover:bg-white/[0.06] transition-all flex flex-col items-center justify-center aspect-square mb-2"><i data-lucide="plus" class="w-8 h-8 text-white/70"></i><span class="text-xs text-white/70 mt-2 font-medium">Buat Playlist</span></div><h3 class="font-semibold text-sm text-white/70 truncate px-0.5">Buat Baru</h3></div>';

        if (S.hp && S.hp.length > 0) {
            S.hp.slice(0, 8).forEach(function(p, i) {
                plHtml += '<div onclick="Album.open(\''+p.id+'\', \''+(p.cover||FI)+'\')" class="flex-shrink-0 w-36 md:w-44 cursor-pointer active:scale-95 transition-all group"><div class="p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl group-hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col"><div class="w-full aspect-square rounded-xl overflow-hidden relative shadow-md mb-2"><img loading="lazy" decoding="async" src="'+(p.cover||FI)+'" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.src=\''+FI+'\'" /></div><h3 class="font-semibold text-sm text-white truncate px-0.5">'+es(p.title)+'</h3><p class="text-xs text-white/60 truncate mt-0.5 px-0.5">'+es(p.artist)+'</p></div></div>';
            });
        }
        ...
        if (a) {
            if (S.ha && S.ha.length > 0) {
                var artHtml = S.ha.slice(0, 10).map(function(p, i) {
                    ...
                    return '<div onclick="Artist.open(\''+p.id+'\', \''+esJs(p.name||p.title)+'\', \''+(p.cover||'')+'\')" class="flex-shrink-0 w-32 cursor-pointer active:scale-95 transition-all group"><div class="p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl group-hover:bg-[rgba(255,255,255,0.1)] transition-all flex flex-col items-center"><div class="w-24 h-24 rounded-full overflow-hidden relative shadow-md mb-2 border-2 border-white/10 group-hover:scale-105 transition-transform duration-300"><img loading="lazy" decoding="async" src="'+imgUrl+'" class="w-full h-full object-cover" onerror="handleImgError(this)" /></div><h3 class="font-semibold text-center text-xs text-white truncate w-full px-0.5">'+es(p.name||p.title)+'</h3><p class="text-white/60 text-[10px] mt-0.5 uppercase tracking-wider font-semibold">Artist</p></div></div>';
                }).join('');
                a.innerHTML = artHtml;
