var Library={
    ...
    render(){
        ...
        var html = '<div class="pt-8 pb-3.5 px-4 sticky top-0 z-30 border-b border-white/[0.08] transition-all cc-header-blur">' +
        ...
        if (isArtistsTab) {
            ...
            if(likedArtists.length === 0){
                ...
                likedArtists.forEach(function(a){
                    html += '<div onclick="Artist.open(\'' + es(a.artistId) + '\', \'' + esJs(a.name) + '\')" class="p-3.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] cursor-pointer active:scale-95 transition-all text-center flex flex-col items-center justify-center group">' +
                        '<div class="relative w-20 h-20 mb-3 rounded-full overflow-hidden border-2 border-white/10 shadow-md group-hover:scale-105 transition-transform duration-300">' +
                            '<img loading="lazy" decoding="async" src="' + a.thumbnail + '" class="w-full h-full object-cover" onerror="this.src=\'' + FI + '\'" />' +
                        '</div>' +
            ...
            if(pls.length === 0){
                ...
                pls.forEach(function(p){
                    html += '<div onclick="Library.open(\'' + p.id + '\')" class="p-2.5 rounded-[22px] backdrop-blur-xl bg-[rgba(255,255,255,0.06)] border border-white/10 shadow-xl hover:bg-[rgba(255,255,255,0.1)] cursor-pointer active:scale-95 transition-all group flex flex-col">' +
                        '<div class="relative w-full aspect-square mb-2.5 rounded-xl overflow-hidden shadow-md">' +
                            '<img loading="lazy" decoding="async" src="' + (p.image || (p.songs.length > 0 ? p.songs[0].cover : FI)) + '" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.src=\'' + FI + '\'" />' +
    ...
    showActions(id){
        ...
        popup.innerHTML='<div class="w-full max-w-md rounded-t-3xl p-6 border-t border-white/10 glass-strong" style="animation:slideUp 0.3s ease-out forwards; background: var(--bg-color);">'+
            ...
            '<div class="flex items-center gap-3 mb-5"><img loading="lazy" decoding="async" src="'+(pl.image||(pl.songs.length>0?pl.songs[0].cover:FI))+'" class="w-12 h-12 rounded-lg object-cover" onerror="this.src=\''+FI+'\'" /><div class="truncate"><h3 class="font-bold text-white truncate">'+es(pl.name)+'</h3><p class="text-white/70 text-xs">'+pl.songs.length+' lagu</p></div></div>'+
    ...
    share(id){
        ...
        var text = 'Dengarkan playlist "' + pl.name + '" (' + pl.songs.length + ' lagu) di Cleo Cloud!';
        if (navigator.share) {
            navigator.share({ title: pl.name + ' - Cleo Cloud', text: text, url: url }).catch(function(){});
        } else {
    ...
    offlineButtonHtml(id){
        var isFull = typeof isPlaylistFullyOffline === 'function' && isPlaylistFullyOffline(id);
        if (isFull) {
            return '<button id="offline-save-all-btn" onclick="showToast(\'Semua lagu di playlist ini sudah tersimpan offline\')" class="text-emerald-400 p-3 rounded-full active:scale-95 bg-emerald-500/10 border border-emerald-500/20 transition-all flex items-center gap-1.5 px-4" title="Tersimpan Offline"><i data-lucide="check-circle-2" class="w-5 h-5"></i><span class="text-xs font-bold">Offline</span></button>';
        }
        return '<button id="offline-save-all-btn" onclick="savePlaylistForOffline(\'' + id + '\')" class="text-white/70 hover:text-white p-3 rounded-full active:scale-95 bg-white/5 transition-all flex items-center gap-1.5 px-4" title="Simpan Semua ke Mode Offline"><i data-lucide="download-cloud" class="w-5 h-5"></i><span class="text-xs font-bold">Offline</span></button>';
    },
    ...
    open(id){
        ...
        if(!modal) {
            ...
            modal.className = 'fixed inset-0 bg-[#050506] flex flex-col z-[100]';
        ...
        var html=`
            ...
            <div class="flex-1 overflow-y-auto hide-scrollbar pb-36 relative" id="library-content" onscroll="Library.handleScroll()">
                <div class="relative w-full aspect-square md:aspect-video max-h-[50vh] overflow-hidden -mt-20 mb-6">
                    <img loading="lazy" decoding="async" src="${pl.image||(pl.songs.length>0?pl.songs[0].cover:FI)}" class="w-full h-full object-cover" onerror="this.src='${FI}'" />
                    <div class="absolute inset-0 bg-gradient-to-t from-[#050506] via-[#050506]/60 to-transparent"></div>
                    <div class="absolute bottom-6 left-6 right-6 flex flex-col justify-end items-center text-center z-10">
                        <img loading="lazy" decoding="async" src="${pl.image||(pl.songs.length>0?pl.songs[0].cover:FI)}" class="w-32 h-32 md:w-48 md:h-48 rounded-xl object-cover border border-white/10 mb-4" onerror="this.src='${FI}'" />
                        <div>
