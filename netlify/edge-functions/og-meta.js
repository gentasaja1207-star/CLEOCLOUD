// ============================================================
// CLEO CLOUD - DYNAMIC OG META INJECTION (Edge Function)
// ============================================================
...
// real preview from. Those links keep the default Cleo Cloud preview.
...
async function getTrackMeta(origin, videoId) {
    ...
    var title = 'Lagu', artist = 'Cleo Cloud', cover = hdCover(null, videoId);
    ...
    return {
        title: title,
        description: 'Dengarkan ' + title + ' - ' + artist + ' di Cleo Cloud',
        image: cover
    };
}
...
async function getArtistMeta(origin, artistId) {
    ...
    return {
        title: name + ' (Artist)',
        description: 'Dengarkan lagu & album terbaik dari ' + name + ' di Cleo Cloud',
        image: image
    };
}
...
async function getAlbumMeta(origin, albumId) {
    ...
    return {
        ...
        description: 'Dengarkan album ' + fullTitle + ' di Cleo Cloud',
        image: image
    };
}
...
function injectMeta(html, meta, pageUrl) {
    html = html.replace(/<title>[^<]*<\/title>/, '<title>' + escapeAttr(meta.title) + ' - Cleo Cloud</title>');
    html = replaceMetaContent(html, /<meta property="og:title"[^>]*>/, meta.title + ' | Cleo Cloud');
