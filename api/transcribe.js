...
const handler = async (req, res) => {
    ...
    if (!url) return res.status(400).json({ creator: 'Genta De Carlo', status: false, message: 'Parameter url atau id diperlukan' });

    try {
        const result = await getTranscribe(url);
        return res.json({
            creator: 'Genta De Carlo',
            status: true,
            result
        });
    } catch (err) {
        return res.status(500).json({ creator: 'Genta De Carlo', status: false, message: err.message });
    }
};
