...
app.use((req, res) => {
    ...
    if (req.path.startsWith('/play/')) {
        ...
        if (videoId) {
            ...
            const playTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist}` : qTitle) : `Dengarkan Musik - Cleo Cloud`;
            const playDesc = `Dengarkan ${qTitle || 'lagu favoritmu'} di Cleo Cloud Web Music Player`;
    ...
    if (req.path.startsWith('/artist/')) {
        ...
        if (artistId) {
            ...
            const pageTitle = qName ? `${qName} (Artist) - Cleo Cloud` : `Artist - Cleo Cloud`;
            const pageDesc = qName ? `Dengarkan lagu & album terbaik dari ${qName} di Cleo Cloud` : `Dengarkan lagu & album dari artist favoritmu di Cleo Cloud`;
    ...
    if (req.path.startsWith('/album/')) {
        ...
        if (albumId) {
            ...
            const pageTitle = qTitle ? (qArtist ? `${qTitle} - ${qArtist} (Album) - Cleo Cloud` : `${qTitle} (Album) - Cleo Cloud`) : `Album - Cleo Cloud`;
            const pageDesc = qTitle ? `Dengarkan album ${qTitle} di Cleo Cloud` : `Dengarkan album favoritmu di Cleo Cloud`;
            const coverUrl = qCover || `https://www.gobox.my.id/file/R0ym4wqfznmp.png`;
